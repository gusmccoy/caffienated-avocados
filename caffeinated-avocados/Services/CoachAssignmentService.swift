...
// The public DB is readable/writable by all authenticated app users regardless
// of iCloud account, making it the right shared medium for cross-user delivery.
//
// Security: each CoachAssignment record is keyed by an inviteCode (8-char random
// UUID fragment) that only the coach and athlete know.  Other users won't query
// the correct code.
//
...
struct CoachAssignmentPayload: Codable {
...
actor CoachAssignmentService {
    ...
    /// Returns every CoachAssignment record for the given invite code so the
    /// caller can add missing workouts and remove locally-deleted ones.
